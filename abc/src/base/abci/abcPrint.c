...
void Abc_NtkPrintStats( Abc_Ntk_t * pNtk, int fFactored, int fSaveBest, int fDumpResult, int fUseLutLib, int fPrintMuxes, int fPower, int fGlitch, int fSkipBuf, int fSkipSmall, int fPrintMem )
{
    ...
    FILE * out_file = fopen("survey_data.txt","w");
    FILE * node_file = fopen("survey_node_data.txt","w");
...
//        Abc_AigCountNext( pNtk->pManFunc );
    ...
    else if ( Abc_NtkIsStrash(pNtk) )
    {
        ...
        fprintf(out_file, "and = %7d", Abc_NtkNodeNum(pNtk));
    ...
    else
    {
        ...
        // Added to print output into file for survey
        fprintf(out_file," nd = %6d", fSkipSmall ? Abc_NtkGetLargeNodeNum(pNtk) : Abc_NtkNodeNum(pNtk) - nSingles);
        fprintf(out_file," edge = %7d", Abc_NtkGetTotalFanins(pNtk) - nSingles );
    }
    ...
    else if ( Abc_NtkHasMapping(pNtk) )
    {
        ...
        // Added for survey
        fprintf(out_file," area = %5.2f", Abc_NtkGetMappedArea(pNtk) );
        fprintf(out_file," delay = %5.2f", Abc_NtkDelayTrace(pNtk, NULL, NULL, 0) );
    ...
    if ( Abc_NtkIsStrash(pNtk) )
    {
        ...
        fprintf(out_file, "  lev = %3d", Abc_AigLevel(pNtk) );
...
//        Abc_Print( 1,"  var = %5d", Abc_NtkCiNum(pNtk) + Abc_NtkCoNum(pNtk)+Abc_NtkGetMultiRefNum(pNtk) );
        ...
        fprintf(out_file,"  lev = %d", Abc_NtkLevel(pNtk) );
    }
    if ( pNtk->nBarBufs2 )
