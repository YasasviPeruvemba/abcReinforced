...
void Abc_NtkTrainingDataPrint(Abc_Ntk_t * pNtk , Abc_Obj_t * pNode, Vec_Ptr_t * vNodes){
...
// traversing through fanin for all nodes
            if(Abc_ObjFaninNum(pNode) > 0){
                ...
                if(Abc_ObjFaninNum(pNode) > maxFanin){
                    maxFanin = Abc_ObjFaninNum(pNode);
                    maxFanin_id = (pNode)->Id;
                }
            }
...
// traversing through fanout for all nodes
            ...
            if(Abc_ObjFanoutNum(pNode) > 0){
                ...
                if(Abc_ObjFanoutNum(pNode) > maxFanout){
                    maxFanout = Abc_ObjFanoutNum(pNode);
                    maxFanout_id = (pNode)->Id;
                }
            }
...
void Abc_NtkShow( Abc_Ntk_t * pNtk0, int fGateNames, int fSeq, int fUseReverse )
{
    ...
    if(Abc_NtkHasMapping(pNtk)){
        Abc_NtkTrainingDataPrint(pNtk , pNode, vNodes);    
    }
    else{
        Abc_Print(-2, "Mapping does not exist.\n");
    }
